<div class="flex gap-4">
            ...
            <form wire:submit='createNewSertifikat' action="#" method="POST" class="space-y-6">
                <div class="flex gap-5">
                    <div class="w-2/4">
                        ...
                        <div>
                            <label for="judul" class="block text-sm/6 font-medium text-gray-900">Judul</label>
                            <div class="mt-2">
                                <input wire:model="judul" id="judul" type="text" name="judul" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
                                @error('judul')
                        ...
                        <div class="mt-5">
                            <label for="nama_pejabat" class="block text-sm/6 font-medium text-gray-900">Nama Pejabat</label>
                            <div class="mt-2">
                                <input wire:model="nama_pejabat" id="nama_pejabat" type="text" name="nama_pejabat" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
                                @error('nama_pejabat')
                                    <p class="text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="mt-5">
                            <label for="nama_jabatan" class="block text-sm/6 font-medium text-gray-900">Nama Jabatan</label>
                            <div class="mt-2">
                                <input wire:model="nama_jabatan" id="nama_jabatan" type="text" name="nama_jabatan" required class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6" />
                                @error('nama_jabatan')
                                    <p class="text-xs text-red-600 dark:text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="w-2/4 space-y-6">
                        ...
                        {{-- ================= LOGO KIRI ================= --}}
                        <div>
                            <label for="logo_kiri" class="block text-sm font-medium text-gray-900">Logo Kiri @if ($isEditing)
                                    <span class="text-xs text-gray-500">(Optional - leave empty to keep current)</span>
                                @endif
                            </label>

                            @if ($isEditing && $old_logo_kiri)
                                <div class="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                    <p class="text-sm text-blue-900 font-medium mb-2">Current Logo Kiri:</p>
                                    <img src="{{ asset('storage/' . $old_logo_kiri) }}" class="rounded w-20 h-20 object-cover">
                                </div>
                            @endif
                            ...
                            <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-300 px-6 py-4">
                                <div class="text-center">
                                    ...
                                    <div class="mt-4 flex text-sm text-gray-600 justify-center">
                                        <label for="logo_kiri" class="relative cursor-pointer font-semibold text-indigo-600 hover:text-indigo-500">
                                            <span>Upload file</span>
                                            <input wire:model="logo_kiri" id="logo_kiri" type="file" name="logo_kiri" @if (!$isEditing) required @endif class="sr-only" accept="image/png, image/jpg, image/jpeg" />
                                        </label>
                                    </div>
                            ...
                            {{-- Loading --}}
                            <div wire:loading wire:target="logo_kiri" class="flex items-center justify-center w-20 h-20 mt-3 border rounded-lg bg-gray-50">
                                <span class="text-xs text-blue-600 animate-pulse">Loading...</span>
                            </div>
                            ...
                            @if ($logo_kiri)
                                <p class="my-2 text-sm/6 font-medium fw-bold">Preview (New)</p>
                                <img src="{{ $logo_kiri->temporaryUrl() }}" class="rounded w-20 h-20 block object-cover">
                            @endif
                        </div>

                        {{-- ================= Tanda Tangan ================= --}}
                        <div>
                            <label for="ttd_pejabat" class="block text-sm font-medium text-gray-900">Tanda Tangan @if ($isEditing)
                                    <span class="text-xs text-gray-500">(Optional - leave empty to keep current)</span>
                                @endif
                            </label>

                            @if ($isEditing && $old_ttd_pejabat)
                                <div class="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                                    <p class="text-sm text-blue-900 font-medium mb-2">Current Tanda Tangan:</p>
                                    <img src="{{ asset('storage/' . $old_ttd_pejabat) }}" class="rounded w-20 h-20 object-cover">
                                </div>
                            @endif
                            ...
                            <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-300 px-6 py-4">
                                <div class="text-center">
                                    ...
                                    <div class="mt-4 flex text-sm text-gray-600 justify-center">
                                        <label for="ttd_pejabat" class="relative cursor-pointer font-semibold text-indigo-600 hover:text-indigo-500">
                                            <span>Upload file</span>
                                            <input wire:model="ttd_pejabat" id="ttd_pejabat" type="file" name="ttd_pejabat" @if (!$isEditing) required @endif class="sr-only" accept="image/png, image/jpg, image/jpeg" />
                                        </label>
                                    </div>
                            ...
                            {{-- Loading --}}
                            <div wire:loading wire:target="ttd_pejabat" class="flex items-center justify-center w-20 h-20 mt-3 border rounded-lg bg-gray-50">
                                <span class="text-xs text-blue-600 animate-pulse">Loading...</span>
                            </div>
                            ...
                            @if ($ttd_pejabat)
                                <p class="my-2 text-sm/6 font-medium fw-bold">Preview (New)</p>
                                <img src="{{ $ttd_pejabat->temporaryUrl() }}" class="rounded w-20 h-20 block object-cover">
                            @endif
                        </div>
    ...
    <div class="w-4/12">
        <div class="bg-white p-4 rounded-lg">
            ...
            <div class="mb-4">
                <input wire:model.live="search" type="text" placeholder="Search by judul sertifikat" class="w-full px-4 py-2 rounded-md border border-gray-300 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-2 focus:outline-offset-2 focus:outline-indigo-600" />
            </div>
                ...
                @forelse ($sertifikats as $sertifikat)
                    <li class="flex justify-between gap-x-6 py-5">
                        <div class="flex min-w-0 gap-x-4">
                            {{-- <img src="{{ $sertifikat->logo_kiri ? asset('storage/' . $sertifikat->logo_kiri) : asset('img/default-avatar.png') }}" alt="" class="size-12 flex-none rounded-full bg-gray-50" /> --}}
                            <div class="min-w-0 flex-auto self-center">
                                <p class="text-sm/6 font-semibold text-gray-900">{{ $sertifikat->judul }}</p>
                            </div>
